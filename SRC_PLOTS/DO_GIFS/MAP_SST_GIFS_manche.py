...
for t in range(len(time_counter)):
    ...
    im1 = plt.contourf(lon, lat, T_VER ,levels=np.linspace(13,20,25), cmap=cmocean.cm.thermal,transform=ccrs.PlateCarree(),extend='both')
    cbar=plt.colorbar(ax=ax,ticks=[np.linspace(13,20,11)],orientation="vertical",fraction=0.046, pad=0.04) #ticks=[-40,20,0,20,40]) #ticks=[0,1,2,3]) #
    cbar.set_label('SST (°C)', rotation=270, labelpad=12, fontsize=12,fontweight="bold")
    im1 = plt.contour(lon, lat, T_VER ,levels=np.linspace(13,20,11), colors='k',linewidths=0.5,transform=ccrs.PlateCarree())
