...
Curl_U=Ffile_VER.socurl.squeeze()[::3,::3]
...
im1 = plt.contourf(lon, lat, Curl_U ,levels=np.linspace(-3e-5,3e-5,51), cmap=mymap,transform=ccrs.PlateCarree(),extend='both')
cbar=plt.colorbar(ax=ax,ticks=[np.linspace(-3e-5,3e-5,11)],orientation="vertical") #ticks=[-40,20,0,20,40]) #ticks=[0,1,2,3]) #
