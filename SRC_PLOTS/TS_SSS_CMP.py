...
T_VER1=Tfile_VER1.vosaline.squeeze().sel(time_counter=slice('DSTART_TS', 'DEND')).mean(dim='x').mean(dim='y')
T_VER2=Tfile_VER2.vosaline.squeeze().sel(time_counter=slice('DSTART_TS', 'DEND')).mean(dim='x').mean(dim='y')
...
plt.title('Salinity',fontsize=20)
plt.xlabel('',fontsize=15)
plt.ylabel('Salinity (kg/kg)',fontsize=15)
plt.legend()
plt.savefig('TS_CFG_NM_SSS_moyenne_VER_DSTART_TS_DEND.png', dpi=300)
