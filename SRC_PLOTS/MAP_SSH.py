...
im1 = plt.contourf(lon, lat, T_VER_m ,levels=np.linspace(-0.3,0.3,51), cmap=mymap,transform=ccrs.PlateCarree(),extend='both')
...
im1 = plt.contour(lon, lat, T_VER_m ,levels=np.linspace(-0.3,0.3,11), colors='k',linewidths=0.5,transform=ccrs.PlateCarree())
ax.coastlines(resolution='50m')

ax.set_title('VER')
plt.tight_layout()
plt.savefig('CFG_NM_MSSH_moyenne_VER_DSTART_DEND.png',dpi=400)
plt.close()
