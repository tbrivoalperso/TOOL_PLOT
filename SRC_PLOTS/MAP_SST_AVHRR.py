...
obs_file=xr.open_mfdataset('/scratch/work/legalloudeco/PERMANENT_SPACE/OBSERVATIONS/SST/AVHRR/ORIGIN_GRID/dt/avhrr-only-v2.*').sel(time=slice('DSTART', 'DEND'))
sst_obs=obs_file.sst.squeeze()[:,:].mean(dim='time')
...
im1 = plt.contourf(lon, lat, sst_obs.T ,levels=np.linspace(9,29,51), cmap=cmocean.cm.thermal,transform=ccrs.PlateCarree(),extend='both')
...
im1 = plt.contour(lon, lat, sst_obs.T ,levels=np.linspace(9,29,11), colors='k',linewidths=0.5,transform=ccrs.PlateCarree())
ax.coastlines(resolution='50m')

ax.set_title('OBS')
plt.tight_layout()
plt.savefig('SST_moyenne_OBS_AVHRR_DSTART_DEND.png',dpi=400)
plt.close()
