...
ssh_2=ssh_2_file.sossheig.squeeze().sel(time_counter=slice('DSTART', 'DEND'))[0:24,:,:]#.stack(xy=("x", "y"))
ssh_2=ssh_2.fillna(0.)

ff_ssh_1=np.zeros(ssh_1.shape)
ff_ssh_1ki0=np.zeros(ssh_1.shape)

ff_ssh_2=np.zeros(ssh_2.shape)
ff_ssh_2ki0=np.zeros(ssh_2.shape)
...
for t in range(len(ssh_1[:,0,0])):
   ...
   for j in range(len(ssh_1[0,0,:])):
       ...
       ff_ssh_2[t,:,j], ff_ssh_2ki0[t,:,j] = signal.welch(ssh_2[t,:,j], fs=dx,nperseg=int(len(ssh_2[0,:,0])), window='hanning', noverlap=no,nfft=2*int(len(ssh_2[0,:,0])-1),  detrend='linear', return_onesided=True, scaling='spectrum')

mean_f0_ssh_1 = np.nanmean(ff_ssh_1,axis=(0,2))
mean_fi0_ssh_1 = np.nanmean(ff_ssh_1ki0,axis=(0,2))

mean_f0_ssh_2 = np.nanmean(ff_ssh_2,axis=(0,2))
mean_fi0_ssh_2 = np.nanmean(ff_ssh_2ki0,axis=(0,2))
...
ax = plt.subplot(111)
ax.plot((1/mean_f0_ssh_1),mean_fi0_ssh_1, 'b-', lw=2, label ='VER1')
ax.plot((1/mean_f0_ssh_2),mean_fi0_ssh_2, 'r-', lw=2, label ='VER2')
