...
Tfile_VER = xr.open_dataset('FLDR/CFG_NM_1d_grid2D.nc', chunks={'time_counter': 50})

T_VER=Tfile_VER.somxlavt.squeeze().sel(time_counter=slice('DSTART', 'DEND')).mean(dim='time_counter')
...
im1 = plt.contourf(lon, lat, T_VER ,levels=np.linspace(0,220,51), cmap="jet",transform=ccrs.PlateCarree(),extend='both')
cbar=plt.colorbar(ax=ax,ticks=np.linspace(0,220,11),orientation="vertical",fraction=0.046, pad=0.04) #ticks=[-40,20,0,20,40]) #ticks=[0,1,2,3]) #
cbar.set_label('MLD (m)', rotation=270, labelpad=12, fontsize=12,fontweight="bold")
ax.coastlines(resolution='50m')

ax.set_title('VER')
plt.tight_layout()

plt.savefig('CFG_NM_MLD_moyenne_VER_DSTART_DEND.png')
plt.close()
