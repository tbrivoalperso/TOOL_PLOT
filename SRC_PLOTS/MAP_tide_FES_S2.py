...
zx=Ufile_FES.S2_z1.squeeze().values
zy=-Ufile_FES.S2_z2.squeeze().values
...
im1 = plt.contourf(lon, lat, amp ,levels=np.linspace(0,1.5,31), cmap='jet',transform=ccrs.PlateCarree(),extend='both')
cbar=plt.colorbar(ax=ax,ticks=np.linspace(0,1.5,11),orientation="vertical") #ticks=[-40,20,0,20,40]) #ticks=[0,1,2,3]) #
cbar.set_label('S2 amplitude (m)', rotation=270, labelpad=12, fontsize=15,fontweight="bold")
########
#
levels=np.arange(-180.,180.,20.)
amp=xr.DataArray(amp,name="amp")
amp.to_netcdf(path="/scratch/work/brivoalt/DATA_eNEATL36/amp_S2_FES.nc")
...
plt.savefig('tide_S2_FES_V2.png')
