...
zx=Ufile_FES.O1_z1.squeeze().values
zy=-Ufile_FES.O1_z2.squeeze().values
...
zx=Ufile_NEMO_4_0.O1_x_elev.squeeze().values
zy=Ufile_NEMO_4_0.O1_y_elev.squeeze().values
zx = np.where(zx>1.e4, np.nan, zx)
zy = np.where(zy>1.e4, np.nan, zy)
zx = np.where(zx==0., np.nan, zx)
zy = np.where(zy==0., np.nan, zy)
...
cbar.set_label('O1 amplitude difference (m)', rotation=270, labelpad=12, fontsize=12,fontweight="bold")
...
plt.savefig('CFG_NM_tide_O1_diff_FES_VER_DSTART_DEND.png')
