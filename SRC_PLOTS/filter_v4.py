...
def filter_file(filepath):
    ...
    if my_file.is_file():
        ...
        ds_tmp=ds.vomecrty.squeeze()
        print(str(filepath) + '.filt4')
        var_V_filt = xr.DataArray( var_V_filt    , coords=ds_tmp.coords, dims=ds_tmp.dims, attrs=ds_tmp.attrs, name='vomecrty_filt4')
        var_V_filt.to_netcdf(str(filepath) + '.filt4')
