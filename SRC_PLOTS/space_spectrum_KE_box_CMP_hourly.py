...
for narea in range(len(listlat_center)):
    ...
    U_1_area=U_1[:,indexy_center_1[narea]- npts_ndeg_1:indexy_center_1[narea]+ npts_ndeg_1,indexx_center_1[narea]- npts_ndeg_1:indexx_center_1[narea]+ npts_ndeg_1]
    V_1_area=V_1[:,indexy_center_1[narea]- npts_ndeg_1:indexy_center_1[narea]+ npts_ndeg_1,indexx_center_1[narea]- npts_ndeg_1:indexx_center_1[narea]+ npts_ndeg_1]
    KE_1_area = 0.5 * (U_1_area**2 + V_1_area**2)
    U_2_area=U_2[:,indexy_center_1[narea]- npts_ndeg_2:indexy_center_1[narea]+ npts_ndeg_2,indexx_center_1[narea]- npts_ndeg_2:indexx_center_1[narea]+ npts_ndeg_2]
    V_2_area=V_2[:,indexy_center_1[narea]- npts_ndeg_2:indexy_center_1[narea]+ npts_ndeg_2,indexx_center_1[narea]- npts_ndeg_2:indexx_center_1[narea]+ npts_ndeg_2]
    ...
    if ISAGRIF1: 
        ...
        U_11_area=U_11[:,indexy_center_11[narea]- npts_ndeg_11:indexy_center_11[narea]+ npts_ndeg_11,indexx_center_11[narea]- npts_ndeg_11:indexx_center_11[narea]+ npts_ndeg_11]
        V_11_area=V_11[:,indexy_center_11[narea]- npts_ndeg_11:indexy_center_11[narea]+ npts_ndeg_11,indexx_center_11[narea]- npts_ndeg_11:indexx_center_11[narea]+ npts_ndeg_11]
    ...
    if ISAGRIF2:
        U_21_area=U_21[:,indexy_center_11[narea]- npts_ndeg_21:indexy_center_11[narea]+ npts_ndeg_21,indexx_center_11[narea]- npts_ndeg_21:indexx_center_11[narea]+ npts_ndeg_21]
        V_21_area=V_21[:,indexy_center_11[narea]- npts_ndeg_21:indexy_center_11[narea]+ npts_ndeg_21,indexx_center_11[narea]- npts_ndeg_21:indexx_center_11[narea]+ npts_ndeg_21]
    ...
    for t in range(len(KE_1_area[:,0,0])):
       print(str(t) + " / " + str(len(KE_1_area[:,0,0])) )
       for j in range(len(KE_1_area[0,0,:])):
           ff_KE_1_area[t,:,j], ff_KE_1_areaki0[t,:,j] = signal.welch(KE_1_area[t,:,j], fs=dx_1*1000,nperseg=int(len(KE_1_area[0,:,0])),\
                                                                        window='hanning', noverlap=no,nfft=2*int(len(KE_1_area[0,:,0])-1),\
                                                                        detrend='linear', return_onesided=True, scaling='spectrum')
           ff_KE_2_area[t,:,j], ff_KE_2_areaki0[t,:,j] = signal.welch(KE_2_area[t,:,j], fs=dx_1*1000,nperseg=int(len(KE_2_area[0,:,0])),\
                                                                        window='hanning', noverlap=no,nfft=2*int(len(KE_2_area[0,:,0])-1),\
                                                                        detrend='linear', return_onesided=True, scaling='spectrum')

       if ISAGRIF1: 
           for j in range(len(KE_11_area[0,0,:])):
               ff_KE_11_area[t,:,j], ff_KE_11_areaki0[t,:,j] = signal.welch(KE_11_area[t,:,j], fs=dx_11*1000,nperseg=int(len(KE_11_area[0,:,0])),\
                                                                              window='hanning', noverlap=no,nfft=2*int(len(KE_11_area[0,:,0])-1),\
                                                                              detrend='linear', return_onesided=True, scaling='spectrum')
        #

               if ISAGRIF2: ff_KE_21_area[t,:,j], ff_KE_21_areaki0[t,:,j] = signal.welch(KE_21_area[t,:,j], fs=dx_11*1000,nperseg=int(len(KE_21_area[0,:,0])),\
                                                                              window='hanning', noverlap=no,nfft=2*int(len(KE_21_area[0,:,0])-1),\
                                                                              detrend='linear', return_onesided=True, scaling='spectrum')
    mean_f0_KE_1_area = np.nanmean(ff_KE_1_area,axis=(0,2))
    mean_fi0_KE_1_area = np.nanmean(ff_KE_1_areaki0,axis=(0,2))

    if ISAGRIF1:
        mean_f0_KE_11_area = np.nanmean(ff_KE_11_area,axis=(0,2))
        mean_fi0_KE_11_area = np.nanmean(ff_KE_11_areaki0,axis=(0,2))
    
    mean_f0_KE_2_area = np.nanmean(ff_KE_2_area,axis=(0,2))
    mean_fi0_KE_2_area = np.nanmean(ff_KE_2_areaki0,axis=(0,2))
    if ISAGRIF2:
        mean_f0_KE_21_area = np.nanmean(ff_KE_21_area,axis=(0,2))
        mean_fi0_KE_21_area = np.nanmean(ff_KE_21_areaki0,axis=(0,2))

    ax = plt.subplot(111)

    ax.plot(mean_f0_KE_1_area,mean_fi0_KE_1_area, 'b', lw=2, label ='CFG_NAME VER1')
    if ISAGRIF1: ax.plot(mean_f0_KE_11_area,mean_fi0_KE_11_area, 'b', lw=2,ls='dotted', label ='AGRIF_CFG_NAME VER1')
    ax.plot(mean_f0_KE_2_area,mean_fi0_KE_2_area, 'r', lw=2, label ='CFG_NAME VER2')
    if ISAGRIF2: ax.plot(mean_f0_KE_21_area,mean_fi0_KE_21_area, 'r', lw=2,ls='dotted', label ='AGRIF_CFG_NAME VER2')
    ...
    ax.plot(1/mean_f0_KE_1_area,mean_fi0_KE_1_area, 'b', lw=2, label ='CFG_NAME VER1')
    if ISAGRIF1: ax.plot(1/mean_f0_KE_11_area,mean_fi0_KE_11_area, 'b', lw=2,ls='dotted', label ='AGRIF_CFG_NAME VER1')

    ax.plot(1/mean_f0_KE_2_area,mean_fi0_KE_2_area, 'r', lw=2, label ='CFG_NAME VER2')
    if ISAGRIF2: ax.plot(1/mean_f0_KE_21_area,mean_fi0_KE_21_area, 'r', lw=2,ls='dotted', label ='AGRIF_CFG_NAME VER2')
    ...
    plt.savefig('CFG_NM_KE_space_spectrum_wavelenght_VER1_VER2_DSTART_DEND'+str(areanames[narea])+'.png')
    plt.close()
