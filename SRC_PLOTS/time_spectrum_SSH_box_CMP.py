...
for narea in range(len(listlat_center)):
    ...
    ssh_obs_area=ssh_obs[:,indexlat_center_obs[narea]-npts_ndeg_obs_lat:indexlat_center_obs[narea]+npts_ndeg_obs_lat,\
                         indexlon_center_obs[narea]-npts_ndeg_obs:indexlon_center_obs[narea]+npts_ndeg_obs].stack(xy=("longitude", "latitude"))
    # Selecting SSH area
    ...
    for i in range(len(ssh_obs_area[0,:])):
       ff_ssh_obs_area[:,i], ff_ssh_obs_areaki0[:,i] = signal.welch(ssh_obs_area[:,i], fs=dx_obs,nperseg=int(len(ssh_obs_area[:,0])),\
                                                                        window='hanning', noverlap=no,nfft=2*int(len(ssh_obs_area[:,0])-1),\
                                                                        detrend='linear', return_onesided=True, scaling='spectrum')
    mean_f0_ssh_obs_area = np.nanmean(ff_ssh_obs_area,axis=1)
    mean_fi0_ssh_obs_area = np.nanmean(ff_ssh_obs_areaki0,axis=1)
    ...
    ax.loglog(mean_f0_ssh_obs_area,mean_fi0_ssh_obs_area, 'k', lw=2, label ='SSH_L4')
    ...
    ax.loglog(1/mean_f0_ssh_obs_area,mean_fi0_ssh_obs_area, 'k', lw=2, label ='SSH L4')
...
for narea in range(len(listlat_center)):
    ...
    print(var_IBI_AGRIF.shape)
    print(indexlon_center_obs[narea] , indexlat_center_obs[narea])
    print(indexlon_center_obs[narea] - npts_ndeg_obs)
    print(indexlon_center_obs[narea] + npts_ndeg_obs)
    npts_ndeg_obs_lat=int(npts_ndeg_obs*np.cos(np.radians(lat_obs[indexlat_center_obs[narea]].values)))
