...
Tfile_VER1 = xr.open_dataset('FLDR_VERSION1/CFG_NM_1d_gridS.nc', chunks={'time_counter': 50})
Tfile_VER2 = xr.open_dataset('FLDR_VERSION2/CFG_NM_1d_gridS.nc', chunks={'time_counter': 50})

T_VER1=Tfile_VER1.vosaline.squeeze().sel(time_counter=slice('DSTART', 'DEND')).mean(dim='time_counter')
T_VER2=Tfile_VER2.vosaline.squeeze().sel(time_counter=slice('DSTART', 'DEND')).mean(dim='time_counter')
...
im1 = plt.contourf(lon, lat, T_VER2 - T_VER1 ,levels=np.linspace(-0.2,0.2,51), cmap=mymap,transform=ccrs.PlateCarree(),extend='both')
...
cbar.set_label('Salinity difference (kg/kg)', rotation=270, labelpad=12, fontsize=12,fontweight="bold")
ax.set_title('VER2 - VER1')

plt.tight_layout()
plt.savefig('CFG_NM_SSS_moyenne_diff_VER2_m_VER1_DSTART_DEND.png')
plt.close()
