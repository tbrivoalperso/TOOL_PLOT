...
def filter_file(filepath):
    ...
    ind_file=1
    print(filepath)
    my_file = Path(filepath +  '.filt20')
    if my_file.is_file():
        print(filepath +  '.filt20' +' already exists, next')
    else:
        ds= xr.open_dataset(filepath).squeeze()
        var_U = ds.sozocrtx.squeeze().values #alues
        var_U_LS = np.zeros(var_U.shape)
        for i in range(len(var_U[:,0,0])):
            var_U_LS[i,:,:] = filter_gaussian(var_U[i,:,:],kernel2D)
        var_U_filt=var_U-var_U_LS
        ds_tmp=ds.vozocrtx
        print(str(filepath) + '.filt20')
        var_U_filt = xr.DataArray( var_U_filt    , coords=ds_tmp.coords, dims=ds_tmp.dims, attrs=ds_tmp.attrs, name='vozocrtx_filt20')
        var_U_filt = var_U_filt.expand_dims(dim='time_counter')
        var_U_filt.to_netcdf(str(filepath) + '.filt20')


print("LETS GO FOR FILTERING!!!!")
filein = sorted(glob.glob(os.path.join('FLDR/', 'CFG_NAME_1h_gridU_????????-????????.nc')))
