...
def filter_file(filepath):
    ...
    if my_file.is_file():
        ...
        var_U_filt = xr.DataArray( var_U_filt    , coords=ds_tmp.coords, dims=ds_tmp.dims, attrs=ds_tmp.attrs, name='vozocrtx_filt12')
        var_U_filt=var_U_filt.expand_dims(dim='time_counter')
        var_U_filt.to_netcdf(str(filepath) + '.filt12')
