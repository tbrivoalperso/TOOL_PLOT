...
def filter_file(filepath):
    ...
    ind_file=1
    print(filepath)
    my_file = Path(filepath +  '.filt4')
    if my_file.is_file():
        print(filepath +  '.filt4' +' already exists, next')
    else:
        ...
        ds_tmp=ds.vozocrtx.squeeze()
        print(str(filepath) + '.filt4')
        var_U_filt = xr.DataArray( var_U_filt    , coords=ds_tmp.coords, dims=ds_tmp.dims, attrs=ds_tmp.attrs, name='vozocrtx_filt4')
        var_U_filt.to_netcdf(str(filepath) + '.filt4')
