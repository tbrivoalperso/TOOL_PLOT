...
def filter_file(filepath):
    ...
    if my_file.is_file():
        ...
        var_V = ds.somecrty.squeeze().values #alues
        var_V_LS = np.zeros(var_V.shape)
        for i in range(len(var_V[:,0,0])):
            var_V_LS[i,:,:] = filter_gaussian(var_V[i,:,:],kernel2D)
        var_V_filt=var_V-var_V_LS
        ds_tmp=ds.vozocrtx
        print(str(filepath) + '.filt20')
        var_V_filt = xr.DataArray( var_V_filt    , coords=ds_tmp.coords, dims=ds_tmp.dims, attrs=ds_tmp.attrs, name='vomecrty_filt20')
        var_V_filt = var_V_filt.expand_dims(dim='time_counter')
        var_V_filt.to_netcdf(str(filepath) + '.filt20')


print("LETS GO FOR FILTERING!!!!")
filein = sorted(glob.glob(os.path.join('FLDR/', 'CFG_NAME_1h_gridV_????????-????????.nc')))
