...
im1 = plt.contourf(lon, lat, ssh_obs.T ,levels=np.linspace(-0.1,0.1,51), cmap=mymap,transform=ccrs.PlateCarree(),extend='both')
...
im1 = plt.contour(lon, lat, ssh_obs.T ,levels=np.linspace(-0.1,0.1,11), colors='k',linewidths=0.5,transform=ccrs.PlateCarree())
ax.coastlines(resolution='50m')

ax.set_title('OBS')
plt.tight_layout()
plt.savefig('MDT_moyenne_OBS_DSTART_DEND_overzoom.png',dpi=400)
plt.close()
