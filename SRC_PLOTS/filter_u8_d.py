...
def filter_file(filepath):
    ...
    ind_file=1
    print(filepath)
    my_file = Path(filepath +  '.filt8')
    if my_file.is_file():
        print(filepath +  '.filt8' +' already exists, next')
    else:
        ...
        ds_tmp=ds.vozocrtx
        print(str(filepath) + '.filt8')
        var_U_filt = xr.DataArray( var_U_filt    , coords=ds_tmp.coords, dims=ds_tmp.dims, attrs=ds_tmp.attrs, name='vozocrtx_filt8')
        var_U_filt=var_U_filt.expand_dims(dim='time_counter')
        var_U_filt.to_netcdf(str(filepath) + '.filt8')
