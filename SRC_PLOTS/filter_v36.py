...
def filter_file(filepath):
    ...
    if my_file.is_file():
        ...
        var_V_filt = xr.DataArray( var_V_filt    , coords=ds_tmp.coords, dims=ds_tmp.dims, attrs=ds_tmp.attrs, name='vomecrty_filt36')
        var_V_filt = var_V_filt.expand_dims(dim='time_counter')
        var_V_filt.to_netcdf(str(filepath) + '.filt36')
